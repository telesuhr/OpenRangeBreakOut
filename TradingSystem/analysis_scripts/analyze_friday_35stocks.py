...
# -*- coding: utf-8 -*-
"""
金曜日推奨ポートフォリオ分析（35銘柄版）

最新の35銘柄×半年間のバックテスト結果から金曜日に強い銘柄を特定
"""
...
def analyze_friday_35stocks(
    result_folder: str = "Output/20251204_235300",
    start_date: str = "2025-06-04"
):
    """金曜日推奨ポートフォリオ分析（35銘柄）"""

    print("=" * 80)
    print("金曜日推奨ポートフォリオ分析（半年間×35銘柄）")
    print("=" * 80)
    ...
    # 推奨ポートフォリオ（厳選基準）
    print("\n" + "=" * 80)
    print("明日（12/5 金曜日）の推奨ポートフォリオ【厳選銘柄】")
    print("=" * 80)
    ...
    recommended = stats_df[
        (stats_df['friday_pf'] > 1.2) &
        (stats_df['half_year_pf'] > 1.0) &
    ...
    print("\n【金曜日 総損益ランキング TOP 15】")
    for i, (_, row) in enumerate(stats_df.head(15).iterrows(), 1):
        friday_pf_str = f"{row['friday_pf']:.2f}" if row['friday_pf'] != float('inf') else "∞"
    ...
    # 金曜日PF < 0.9 かつ トレード数 >= 15（統計的に信頼できる弱さ）
    avoid_stocks = stats_df[
        (stats_df['friday_pf'] < 0.9) &
        (stats_df['friday_trades'] >= 15)
    ].head(10)

    if len(avoid_stocks) > 0:
        print(f"\n❌ 回避推奨: {len(avoid_stocks)}銘柄（金曜日PF < 0.9）")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        analyze_friday_35stocks(result_folder, start_date)
    else:
