"""
3ヶ月間の日次パフォーマンス分析スクリプト

最適パラメータ（profit_target=4.0%, stop_loss=0.5%）で
テクノロジー・通信セクターの日次損益を詳細に出力
"""
import logging
...
def main():
    ...
    # 対象銘柄（テクノロジー・通信セクター）
    tech_symbols = SECTORS['テクノロジー・通信']
    ...
    print("=" * 100)
    print("3ヶ月間 日次パフォーマンス分析")
    print("=" * 100)
    print(f"期間: {start_date.date()} - {end_date.date()}")
    print(f"対象セクター: テクノロジー・通信 ({len(tech_symbols)}銘柄)")
    print(f"利益確定: {params['profit_target']:.1%}、損切り: {params['stop_loss']:.1%}")
    print(f"初期資金: {params['initial_capital']:,}円 × {len(tech_symbols)}銘柄")
    print("=" * 100)
    print()
    ...
    for idx, symbol in enumerate(tech_symbols, 1):
        print(f"\r[{idx}/{len(tech_symbols)}] {STOCK_NAMES.get(symbol, symbol):25s}",
              end='', flush=True)
    ...
    total_investment = params['initial_capital'] * len(tech_symbols)
    ...
    # CSV出力
    csv_filename = f"results/optimization/daily_performance_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.csv"
