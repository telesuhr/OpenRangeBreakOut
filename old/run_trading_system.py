...
def main():
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info(f"\nレポート出力先: {config['reports']['output_dir']}/")
        logger.info(f"  - 日次レポート: {config['reports']['output_dir']}/daily_reports/")
        logger.info(f"  - チャート: {config['reports']['output_dir']}/charts/")
        logger.info(f"  - サマリー: {config['reports']['output_dir']}/summary/")
